...
export class HordesScene extends Phaser.Scene {
  ...
  private enemyBaseHp = DEFAULT_MOB.health
  ...
  update(_time: number, delta: number) {
    ...
    this.enemies = this.enemies.filter((enemy) => {
      ...
      if (this.combat.applyAuraDamage(enemy, enemyRadius, dist, mob)) {
        return false
      }
  ...
  private spawnWave() {
    ...
    this.wave += 1
    this.updateHud()
    const count = Math.min(2 + this.wave, 10)
